       ...
       if (produto == null) {
        ...
        if (produto.getNome()== null || "".equals(produto.getNome())) {
         ...
         if (produto.getTamanho()== null || "".equals(produto.getTamanho())
                || (!produto.getTamanho().equals("P"))
                || (!produto.getTamanho().equals("M"))
                || (!produto.getTamanho().equals("G"))
                && !produto.getTamanho().equals("GG")) {
            throw new ClienteException("É necessário informar o "
                    + "tamanho do produto");
           }
          if (produto.getCor()== null || "".equals(produto.getCor())
                || (!produto.getCor().equals("Preto"))
                || (!produto.getCor().equals("Branco"))
                || (!produto.getCor().equals("Azul"))  
                && !produto.getCor().equals("Vermelho")) {
